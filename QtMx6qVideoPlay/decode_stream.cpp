...
int ReadBitstream(DecContxt * pDecContxt, unsigned char* pBitstream,int length)
{
	...
	readbytes = pDecContxt->fincallback(pBitstream,length,pDecContxt->uiPara);
...
int OutputFrame(DecContxt * pDecContxt,VpuDecOutFrameInfo* pOutFrame,int width, int height,int fbHandle,int frameNum)
{
	...
	if((pDecContxt->nMapType!=0) && (pDecContxt->nTile2LinearEnable==0)
		&& (pDecContxt->foutcallback || fbHandle))
	{
		NeedConvert=1;
	}
	...
	if(pDecContxt->foutcallback)
	{
		memcpy(pDecContxt->yuvbuf,pFrame->pbufVirtY,ySize);
		memcpy(pDecContxt->yuvbuf+ySize,pFrame->pbufVirtCb,uvSize);
		memcpy(pDecContxt->yuvbuf+ySize+uvSize,pFrame->pbufVirtCr,uvSize);
		pDecContxt->foutcallback(pDecContxt->yuvbuf,ySize+uvSize*2,width,height,pDecContxt->uiPara);
...
int decode_stream(DecContxt * pDecContxt)
{
	...
	pBitstream=(unsigned char*)malloc(nUnitDataSize);
	if(NULL==pBitstream)
	{
